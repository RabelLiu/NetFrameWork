    ...
    private static class DownloadHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            case PAUSE_TASK: {
                ...
                try {
                    DownloadTask newTask = new DownloadTask(task.getContext(), dnMgr, task.getUrl(),
                            task.getFileName(), task.getPath(), task.getListener());
            ...
            case DELETE_TASK: {
                ...
                // __tq
                File fileTemp = new File(task.getPath(), task.getFileName() + DownloadTask.TEMP_SUFFIX);
                if (fileTemp.exists()) {
                    Log.d("Debug", "delete fileTemp:" + task.getPath() + "/" + task.getFileName()
                            + DownloadTask.TEMP_SUFFIX);
                    fileTemp.delete();
                }
                File file = new File(task.getPath(), task.getFileName());
                if (file.exists()) {
                    Log.d("Debug", "delete file:" + task.getPath() + "/" + task.getFileName());
                    file.delete();
                }
